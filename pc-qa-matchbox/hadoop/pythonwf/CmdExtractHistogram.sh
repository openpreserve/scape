...
hdfs dfs -rm -r $hdfshome/histogram 

# pass input file list and extact SIFT features using Hadoop streaming API
printf "+++ pass input file list and extact visual histograms using Hadoop streaming API"

export HADOOP_HOME=/usr/lib/hadoop

hadoop jar /usr/lib/hadoop-0.20-mapreduce/contrib/streaming/hadoop-*streaming*.jar -D mapred.reduce.tasks=0 -input inputfiles -inputformat org.apache.hadoop.mapred.lib.NLineInputFormat -file $localhome/ExtractHistogram.sh -mapper $localhome/ExtractHistogram.sh -output histogram
