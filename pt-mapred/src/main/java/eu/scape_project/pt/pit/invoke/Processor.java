...
public class Processor {
    ...
    /**
     * Factory method to create a processor from toolspec and action
     * identifiers,
     *
     * @param toolspec_id
     * @param action_id
	...
	/*
     * public static Processor createProcessor( String toolspec_id, String
     * action_id ) throws ToolSpecNotFoundException, CommandNotFoundException {
     * try { ToolSpec ts = ToolSpec.fromInputStream(
     * ToolSpec.class.getResourceAsStream("/toolspecs/"+toolspec_id+".ptspec.xml"));
     * Action action = Processor.findTool(ts, action_id); // Create the right
     * class: if( "identify".equals( action.getType() ) ) { return new
     * Identify(ts,action); } else { return new Processor(ts,action); } } catch
     * (FileNotFoundException e) { throw new ToolSpecNotFoundException("Toolspec
     * "+toolspec_id+" not found!", e); } catch (JAXBException e) { throw new
     * ToolSpecNotFoundException("Toolspec "+toolspec_id+" not parseable!", e);
     * } catch (CommandNotFoundException e) { // TODO Auto-generated catch block
    ...
    public static Action findTool(ToolSpec ts, String command_id) throws CommandNotFoundException {
        ...
        for (Action c : ts.getActions()) {
            if (c.getId() != null && c.getId().equals(command_id)) {
                cmd = c;
            }
        }
        if (cmd == null) {
            throw new CommandNotFoundException("No command " + command_id + " could be found.");
        }
        return cmd;
    }
    ...
    protected void runCommand(String[] cmd_template) throws IOException {
        ...
        /*
         * for( String command : pb.command() ) { System.out.println("Command :
         * "+command); }
         */
        ...
        try {
            ...
            if (this.stdin != null) {
                //IOUtils.copyLarge(this.stdin, start.getOutputStream());
                byte[] buffer = new byte[1024];
                int len;
                int c = 0;
                while ((len = this.stdin.read(buffer)) != -1) {
                    c += len;                    
                    System.out.println("len = " + len + "; c = " + c );
                    start.getOutputStream().write(buffer, 0, len);                                        
                }
            ...
            if (this.stdout != null) {
                //IOUtils.copy(procStdout, this.stdout);
                byte[] buffer = new byte[1024];
                int len;
                while ((len = procStdout.read(buffer)) != -1) {
                    this.stdout.write(buffer, 0, len);
                }
            } else {
    ...
    */
    ...
    private void setStdin(InputStream input ) {
        this.stdin = input;
    ...
    private void setStdout( OutputStream output ) {
        this.stdout = output;
    }
     ...
     * @throws CommandNotFoundException
     */
    public void execute(HashMap<String, Stream> parameters) throws IOException, CommandNotFoundException {
        ...
        if (parameters != null) //vars.putAll(parameters);
        {
            ...
            Stream istream = parameters.get("input");
            Stream ostream = parameters.get("output");
            
            if (getUseStdin()) {  
                System.out.println( this.getClass() + ": using StdIn");
                this.setStdin( istream.getInputStream() );
                
                // FIXME should we also assume stdout if "useStdIn"?
                this.setStdout( ostream.getOutputStream() );
            }
            else
            {
                // FIXME workaround: using "input" and "output" by convention
                File ifile = istream.getFile();
                vars.put("input", ifile.getAbsolutePath() );                
                
                // FIXME that part won't work!
                File ofile = ostream.getFile();
                vars.put("output", ofile.getAbsolutePath() );
            }
        }
    ...
    public boolean getUseStdin() {        
        if (action.getInputs() != null) {
            return action.getInputs().getUseStdin();
        }
        if (ts.getInputs() != null) {
            return ts.getInputs().getUseStdin();
        }
        return false;
    }
