...
import eu.scape_project.pt.proc.JavaProcessor;
 ...
 * 
 */
public class ToolspecMapper extends
		Mapper<LongWritable, Text, LongWritable, Text> {

	private static Log LOG = LogFactory.getLog(ToolspecMapper.class);
	public static String JAVA_TOKEN = "java:";
	...
	public void map(LongWritable key, Text value, Context context)
			throws IOException {
		LOG.debug("Mapper.map key:" + key.toString() + " value:"
				+ value.toString());
		...
		try {
			...
			for (int c = 0; c < commands.length; c++) {
				...
				// check if action starts with java:
				String strAction = this.operation.getCommand().toLowerCase()
						.trim();
				LOG.debug("Action string is: " + strAction);

				if (strAction.startsWith(JAVA_TOKEN)) {
					LOG.debug("Discovered Java action: " + strAction);
					lastProcessor = new JavaProcessor(this.tool);
					lastProcessor.findOperation(command.action).setCommand(
							strAction.substring(JAVA_TOKEN.length(),
									strAction.length()));
				}

				if (this.operation == null)
					throw new IOException("operation " + command.action
							+ " not found");
				...
				mapInputFileParameters[c] = lastProcessor
						.getInputFileParameters();
				mapOutputFileParameters[c] = lastProcessor
						.getOutputFileParameters();

				// copy parameters to temporal map
				Map<String, String> mapTempInputFileParameters = new HashMap<String, String>(
						mapInputFileParameters[c]);
				Map<String, String> mapTempOutputFileParameters = new HashMap<String, String>(
						mapOutputFileParameters[c]);

				// localize parameters
				for (Entry<String, String> entry : mapInputFileParameters[c]
						.entrySet()) {
					...
					for (String file : filelist) {
						...
						filer.setDirectory(context.getTaskAttemptID()
								.toString());
						filer.localize();
						mapTempInputFileParameters.put(entry.getKey(),
								filer.getFileRef());
					}
				}

				for (Entry<String, String> entry : mapOutputFileParameters[c]
						.entrySet()) {
					...
					for (String file : filelist) {
						...
						filer.setDirectory(context.getTaskAttemptID()
								.toString());
						mapTempOutputFileParameters.put(entry.getKey(),
								filer.getFileRef());
					}
				}
				...
				lastProcessor
						.setInputFileParameters(mapTempInputFileParameters);
				lastProcessor
						.setOutputFileParameters(mapTempOutputFileParameters);
			...
			if (strStdinFile != null) {
				InputStream iStdin = Filer.create(strStdinFile)
						.getInputStream();
			...
			for (int i = 0; i < mapOutputFileParameters.length; i++)
				for (String strFile : mapOutputFileParameters[i].values()) {
					LOG.debug("delocalizing file: " + strFile);
